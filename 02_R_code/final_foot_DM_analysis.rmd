...
Coverage <- Me + Un
# Prefiltered total # of CpGs sequenced: 
colSums(Coverage > 0, na.rm = TRUE) 
head(Coverage)

## Calculating # of unique CpGs 
covered_per_cpg <- rowSums(Coverage > 0, na.rm = TRUE)
sum(covered_per_cpg > 0) #1972678

#prefiltered coverage
Coverage_prefilter<-Un + Me
Coverage_prefilter[Coverage_prefilter == 0] <- NA
mean_coverage_prefilter <- colMeans(Coverage_prefilter, na.rm = TRUE)
mean_coverage_prefilter
...
#Postfiltered total # of CpGs: 
colSums(!is.na(prop_meth_matrix_foot))
...
wilcox_effsize<-wilcox_effsize(data = lrt_trans_foot$table,
  formula = logFC ~ 1, mu = 0)
...
# write_csv(all_CpG_dm_foot ,"/Users/qcai/mytilus_github/Data/foot_unique_all_CpG.csv")
...
unzip("/Users/qcai/mytilus_github/Data/new_genomic_intron.gff.zip",
      exdir = "/Users/qcai/mytilus_github/Data/")

gff_intron<-read.gff("/Users/qcai/mytilus_github/Data/new_genomic_intron.gff")

### Convert the gff object to a GRanges object using the gffToGRanges() function of the R package genomation ###
all_GRange<-gffToGRanges("/Users/qcai/mytilus_github/Data/new_genomic_intron.gff", filter = NULL, zero.based = FALSE, ensembl = FALSE) 
...
### UTR subset ###
utr_subset_foot<-subsetByOverlaps(CpGs_foot, UTR_GRange)
#intron_subset_foot<-as.data.frame(intron_subset_foot) 

### Adding gene id ###
utr_gff <- subset(all_GRange, type %in% c("five_prime_UTR", "three_prime_UTR")) #need to be GRange object to subset
mcols(intron_gff)
utr_id_overlaps_foot <- findOverlaps(utr_subset_foot, utr_gff)

### Extract gene_id for matching UTRs from the GFF file ###
gene_ids_foot<- mcols(utr_gff)$gene[subjectHits(utr_id_overlaps_foot)]
query_hits <- queryHits(utr_id_overlaps_foot)

### Add gene_id only to those rows in utr_subset that have corresponding overlaps ###
### Initialize with NA for those rows that have no overlap ###
utr_subset_gene_id_foot <- rep(NA, length(utr_subset_foot))

### Assign the gene_ids where there is a match ###
utr_subset_gene_id_foot[query_hits] <- gene_ids_foot

### Add gene_id as a metadata column to utr_subset df ###
mcols(utr_subset_foot)$gene_id <- utr_subset_gene_id_foot
utr_subset_gene_id_df_foot<-as.data.frame(utr_subset_foot)
utr_subset_gene_id_df_foot$combined <- paste(utr_subset_gene_id_df_foot$seqnames, utr_subset_gene_id_df_foot$start, sep = "-") #reformat
...
#### Creating a dataframe with all the CpG sites, the ones that are differentially methylated with transplant site were "1" and non-methylated sites are "0" ###
...
### Adding genic features to the data frame for each site ###
intron_subset_foot<-intron_subset_gene_id_df_foot %>% 
  mutate(feature = "intron") #29203 obs

exon_subset_foot<-exon_subset_gene_id_df_foot %>% 
  mutate(feature = "exon") #5819 obs

intergenic_subset_foot<-interg_subset_df_foot %>% 
  mutate(feature = "interg") #45096 obs (this number includes overlap with promoter regions)

promoter_subset_foot<-promoter_subset_gene_id_df_foot %>% 
  mutate(feature = "promoter") #2029 obs

utr_subset_foot<-utr_subset_gene_id_df_foot %>% 
  mutate(feature = "UTR") #863 obs
...
### Finding CpG subsets overlapped with UTR regions ###
utr<-CpGsite_foot %>%
  full_join(utr_subset_foot, by = c("Chr" = "seqnames", "Locus" = "start")) %>%
na.omit()

### Combining all the CpG subsets ###
df_CpGsite_foot_all <- bind_rows(exon, intron, intergenic, promoter,utr)
...
### Remove intergenic regions that overlap with promoter regions ###
 ...
 filter(!(any(feature == "promoter") & feature == "interg")) %>%
  ungroup() #intergenic count: 43525
...
### no DM on promoter regions, thus excluding all the promoter entries for the GLM tests, 79410 left
df_combined_foot_trans_df <- df_combined_foot_trans_df %>% filter(feature != "promoter")

glm <- glm(Meth.x ~ feature , data =  df_combined_foot_trans_df, family = binomial(link = "logit"))
summary(glm)
car::Anova(glm, type ="III",test.statistic="LR")

#pairwise comparison
emmeans(glm, pairwise ~ feature, type="response")
...
unzip(
  "/Users/qcai/mytilus_github/Data/GCF_021869535.1_xbMytCali1.0.p_gene_ontology.gaf.zip",
  exdir = "/Users/qcai/mytilus_github/Data/"
)
...
go_terms<-
  read.delim(
    "/Users/qcai/mytilus_github/Data/GCF_021869535.1_xbMytCali1.0.p_gene_ontology.gaf",
    header = FALSE,
    sep = "\t",
...
origin_combined <- unique(updated_origin_gene_unique$combined) #96
transplant_combined <- unique(updated_transplant_gene_unique$combined) #83
shared_combined <- intersect(origin_combined, transplant_combined)
# Count how many are shared
length(shared_combined)
#28

# Extracting the same entries:
# origin_shared_rows <- updated_origin_gene_unique[
#   updated_origin_gene_unique$combined %in% shared_combined, ]
# transplant_shared_rows <- updated_transplant_gene_unique[
#   updated_transplant_gene_unique$combined %in% shared_combined, ]
# shared_rows_all <- rbind(origin_shared_rows, transplant_shared_rows) %>% 
#   filter(Treat=="Origin")

#### Making input file for comparison ###
...
# write_csv(merged_all_DM_foot,"/Users/qcai/mytilus_github/Data/merged_all_DM_foot.csv")
# 
...
enriched.GO <- GO.wall$category[p.adjust(GO.wall$over_represented_pvalue, method = "BH" ) < 0.05]
 head(enriched.GO)
 #character(0)
 # write_csv(GO.wall, "/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/goseq_trans.csv")
 
 
  write_csv(GO.wall, "/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/goseq_origin.csv")
...
write_csv(average_lfg_foot_transplant_filtered,"/Users/qcai/mytilus_github/Outputs/lfg_files/average_lfg_foot_transplant_filtered_no_interaction.csv")
...
write_csv(average_lfg_foot_origin_filtered,"/Users/qcai/mytilus_github/Outputs/lfg_files/average_lfg_foot_origin_filtered_no_interaction.csv")
...
write.table(
  go_foot_df,
  "/Users/qcai/mytilus_github/Outputs/lfg_files/GO_foot_df.tab",
  row.names = FALSE,
  sep = "\t",
  quote = FALSE,
  col.names = FALSE
