...
setwd("/Users/qcai/mytilus_github/Data/gill_coverage_files")
...
## unzip Archive folder before moving on, folder contains all .cov files
 untar("gill_samples.tar.gz", exdir = "archive/")
 
meta_data_gill<-read.delim("gill_metadata.txt", row.names = "sample", stringsAsFactors = FALSE)
Sample_gill <- row.names(meta_data_gill)
files_gill <- paste0("archive/gill_samples/", Sample_gill,".CpG_report.merged_CpG_evidence.cov.CpG_report.merged_CpG_evidence.cov")
...
sum(covered_per_cpg > 0) # 2,485,629
...
### Calculating post-filtered genome-wide coverage 
...
colSums(!is.na(prop_meth_matrix_gill))   
...
effet_size <- wilcox_effsize(data = lrt_trans_gill$table,
  formula = logFC ~ 1, mu = 0) #effect size 0.06989023

wilcox_effsize(data = lrt_origin_gill$table,
  formula = logFC ~ 1, mu = 0)  #effect size: 0.0512613
...
### UTR subset ###
utr_subset_gill<-subsetByOverlaps(CpGs_gill, UTR_GRange)
#intron_subset_gill<-as.data.frame(intron_subset_gill) 

### Adding gene id ###
utr_gff <- subset(all_GRange, type %in% c("five_prime_UTR", "three_prime_UTR")) #need to be GRange object to subset
mcols(intron_gff)
utr_id_overlaps_gill <- findOverlaps(utr_subset_gill, utr_gff)

### Extract gene_id for matching UTRs from the GFF file ###
gene_ids_gill<- mcols(utr_gff)$gene[subjectHits(utr_id_overlaps_gill)]
query_hits <- queryHits(utr_id_overlaps_gill)

### Add gene_id only to those rows in utr_subset that have corresponding overlaps ###
### Initialize with NA for those rows that have no overlap ###
utr_subset_gene_id_gill <- rep(NA, length(utr_subset_gill))

### Assign the gene_ids where there is a match ###
utr_subset_gene_id_gill[query_hits] <- gene_ids_gill

### Add gene_id as a metadata column to utr_subset df ###
mcols(utr_subset_gill)$gene_id <- utr_subset_gene_id_gill
utr_subset_gene_id_df_gill<-as.data.frame(utr_subset_gill)
utr_subset_gene_id_df_gill$combined <- paste(utr_subset_gene_id_df_gill$seqnames, utr_subset_gene_id_df_gill$start, sep = "-") #reformat
...
utr_subset_gill<-utr_subset_gene_id_df_gill %>%
  mutate(feature = "UTR") #801
...
### Finding CpG subsets overlapped with UTRs ###
utr <-CpGsite_gill %>%
  full_join(utr_subset_gill, by = c("Chr" = "seqnames", "Locus" = "start")) %>% na.omit()

### Combining all the CpG subsets ###
df_CpGsite_gill_all <- bind_rows(exon, intron, intergenic, promoter, utr)
...
glm <- glm(Meth.x ~ feature , data =  df_combined_gill_trans_df, family = binomial(link = "logit"))
summary(glm)
