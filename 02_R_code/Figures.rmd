...
meta_data<-read.csv("sample_65_wlength.csv")
...
lm_length_PC1<-lmer(PC1~`LENGTH_FINAL..mm.` + tissue + (1|`meth`), data=pca_data)
summary(lm_length_PC1)

### GETTING R^2 VALLUES
r.squaredGLMM(lm_length_PC1)

lm_length_PC2<-lmer(PC2~`LENGTH_FINAL..mm.` + tissue + (1|`meth`), data=pca_data)
...
<!-- pc1_length<-ggplot(pca_data, aes(x = `LENGTH_FINAL..mm.`, y = PC1)) + #plotting shell length and PC1 loadings -->
<!--   geom_point(aes(fill= tissue), alpha = 0.8, stroke = 0.2, col="black", shape =21, -->
...
pc1_length <- ggplot(pca_data, aes(x = `LENGTH_FINAL..mm.`, y = PC1)) +
  geom_ribbon(
    data = prd,
    aes(x = x, ymin = conf.low, ymax = conf.high),
    fill = "grey",
  ...
  geom_point(
    aes(fill = tissue, shape = GROUP_CODE),
    size = 5,
    alpha = 0.8,
    stroke = 0.2,
    color = "black"
...
combined <- (p1_fix | guide_area() | p2_fix) +
 ...
 <!-- ggsave( -->
 <!--   "/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/fig2.png", -->
 <!--   plot = combined, width = 12, height = 4.8, dpi = 300, bg = "white" -->
 <!-- ) -->
...
<!-- pc2_length<-ggplot(pca_data, aes(x = `LENGTH_FINAL..mm.`, y = PC2)) + #plotting shell length and PC1 loadings -->
<!--    geom_point(aes(fill= tissue), alpha = 0.8, stroke = 0.2, col="black", shape =21, size=6) +  # Keep points colored by tissue -->
<!--    geom_line(aes(group = `meth`), color = "black", linetype = 1) +  -->
...
pc2_length <- ggplot(pca_data, aes(x = `LENGTH_FINAL..mm.`, y = PC2)) +
  geom_ribbon(
    data = prd2,
    aes(x = x, ymin = conf.low, ymax = conf.high),
    fill = "grey",
...
ggsave("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/figS2_updated.png", plot = pc2_length, width = 12, height = 8, dpi = 300)
...
#get the results stats in response scale:
#baseline value: plogis(-2.405639) --> 0.0827437
#with the effect: plogis(-2.405639  +  0.029291) --> 0.08499415
...
p1 <- ggplot(meth_prop_all, aes(x = `Transplant site`, y =`Pre-filtered mean genome wide methylation (%)`/100, fill = Tissue)) + 
...
### just jitter with mean and SD
p2 <- ggplot(meth_prop_all, aes(x = `Transplant site`, y =`Post-filtered mean genomw wide methylation (%)`/100, fill = Tissue)) + 
  # Jittered points with custom appearance
  geom_jitter( size = 6, alpha = 0.6, 
               stroke = 0.2, shape = 21, 
               aes(fill = Tissue, color = "black"),
...
# ggsave("global_meth_genomewide_prefilter.pdf", p1, device = "pdf", width = 12, height = 5, dpi = 300, units = "in")
 ggsave("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/global_meth_genomewide_prefilter_revised.png", p1, device = "png", width = 12, height = 8, dpi = 300, units = "in")
# ggsave("global_meth_genomewide_postfilter.pdf", p2, device = "pdf", width = 14, height = 10, dpi = 300, units = "in")
 ggsave("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/global_meth_genomewide_postfilter.png", p2, device = "png", width = 12, height = 8, dpi = 300, units = "in")
...
png("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/final_DM_combined_no_interaction_updated.png", width = 12, height = 10, units = "in", res = 300)
print(combined)
...
#### Code to make Figure 4 in the main text - bar plot with proportion data ###
#In Fisher_test.R


#### Code to make Figure 5 in the main text, as well as code for testing correlation of overall log2FC values between the two tissues ###
...
foot_origin      <-  96  # area 1
foot_transplant  <-  83  # area 2
foot_ori_trans   <- 28    # intersection (area1 ∩ area2)
...
#compare foot and gill
#look at # of total CpG overlap
length(intersect(unique(df_CpGsite_foot_all$combined),unique(df_CpGsite_gill_all$combined)))

#origin site overlap
intersect(df_combined_gill_origin_df_DM$combined,df_combined_foot_origin_df_DM$combined)

#transplant overlap
intersect(df_combined_gill_trans_df_DM$combined,df_combined_foot_trans_df_DM$combined)


#### Pearson's correlation between all logFC in foot and gills ####

#load data
gill_all_logfc<-read_csv("/Users/qcai/mytilus_github/Data/gill_unique_all_CpG.csv")
foot_all_logfc<-read_csv("/Users/qcai/mytilus_github/Data/foot_unique_all_CpG.csv")
...
merged_long <- merged %>%
  dplyr::select(sample,
         Gill_Transplant, Foot_Transplant,
         Gill_Origin, Foot_Origin) %>%
  pivot_longer(
    cols = -sample,
    names_to = c("Tissue", "Condition"),
    names_sep = "_",
    values_to = "value"
  ) %>%
...
combined_plot <- ggplot(merged_long,
                        aes(x = Gill, y = Foot)) +
  geom_point(alpha = 0.5, color = "black", size = 1.3) +
  geom_smooth(method = "lm", se = FALSE,
              col = "red", size = 1) +
  facet_wrap(~Condition, nrow = 1) +   # puts Transplant & Origin on top
  theme_few(base_size = 15) +
  labs(x = "Gill log2FC",
       y = "Foot log2FC") +
...
ggsave("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Round1_revision/figS5_cor_foot_gill.png",combined_plot, height =6, width=12)
