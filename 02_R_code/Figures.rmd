...
pc_variance <- read.csv(
  "/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/Methymytilus_Final_Rcode_github/PC_variance.csv",
  row.names = 1,
  check.names = FALSE
)
pc <- data.frame(
...
coords <- edistance %>%
  select(X, Tissue, PC1, PC2)

# making all pairwise combinations
pair_indices <- combn(nrow(coords), 2)

#between differnt tissues of different individuals
...
scores19 <- as.matrix(pc_variance[paste0("PC", 1:19)])
D <- dist(scores19, method = "euclidean")
...
# PCA distance matrix across all samples 19 PCs
pca_dist_19 <- dist(pca_dist, method = "euclidean")

mantel_test <- mantel(pca_dist_19, id_dist,
                      method = "pearson", permutations = 999)

mantel_test

# PCA distance matrix across all samples for PC1 and PC2
...
pc1_length<-ggplot(PC, aes(x = `LENGTH_FINAL..mm.`, y = PC1)) + #plotting shell length and PC1 loadings
  ...
  scale_fill_manual(values = c("Foot" = "#3bbdcc", "Gills" = "#e68d4e"),
...
ggsave(
  "/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/manuscript/submission/fig2_updated.png",
...
meth_prop_all<-read_csv("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/cg_coverage_files/tissue_specific/foot_final/percentage_allsample_genomewide.csv")
...
p1 <- ggplot(meth_prop_all_pre, aes(x = `Transplant site`, y =`Pre-filtered mean genome wide methylation (%)`/100, fill = Tissue)) + 
  ...
  scale_fill_manual(values = c("Foot" = "#3bbdcc", "Gills" = "#e68d4e")) +  # Custom fill colors for Tissue
  scale_color_manual(values = c("Foot" = "#2c8e99", "Gills" = "#d4562a")) +  # Custom point color for Tissue
   guides(color = "none")+
...
gill_volcano_labeled <- gill_volcano + plot_annotation(title = "B. Gills",
                                                       theme = theme(plot.title = element_text(face= "bold", size = 20)))
