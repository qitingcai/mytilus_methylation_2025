...
#filtering n=3 for 66% of samples, total of 14 samples
n=3
...
#proportion methylation matrix after filtering
...
# Merge transplant site and origin site coefficients
...
# separating origin site and transplant site effect- volcano plots
...
# Identify genomic features and assess their relationship with DM CpGs----

#gff with intron annotated, output from agat
gff_intron<-read.gff("/Users/qcai/Documents/UCSC/Kelley_Lab/mytilus/cg_coverage_files/TOP_5/new_genomic_intron.gff")

# 2. convert the gff object to a GRanges object using the gffToGRanges() function of the R package genomation. see /hb/groups/kelley_lab/tina/mytilus/ref_genome
...
# 3. Divide the annotation GRanges object into different objects filtered for exons and introns
...
# For negative strand, get the first exon with the largest start position (start from right to left)
...
# Remove potential duplicates again due to overlapping first exons
...
#for some first exons start site <1000bp, 1kb upstream are negative values, adjust length that the exons with start position less than 1000 has a start site of 1 instead of negative values

# Adjust start positions for promoter regions based on strand
adjusted_start <- ifelse(strand(first_exon_GRange) == "+",
                         start(first_exon_GRange) - 1001,  # Positive strand: upstream 1kb
                         end(first_exon_GRange)+1)    # Negative strand: one bp pass the end of first exon
...
#4. Convert a data frame of coordinates of your CpGs to a GRange object using the function makeGRangesFromDataFrame() of the R package GenomicRanges. The data frame will need to be formatted like a GRanges object.

#use filtered prop_meth_matrix from above, extract first rows as the position, format like "NW_026262581.1-97825"
prop_meth_matrix_gill <- Me_gill/(Me_gill+Un_gill)
...
#making CpG subsets according to genic features
#exons
...
#loading the updated DM csv for gill

#all CpGs are getting from y_gill
...
#testing for origin site effect DM
...
#getting dataframe with all the CpG sites, the ones that are differentially methylated with transplant site were "1" and nonmeth are "0", separate analysis for transplant site and origin site effect. replace left join
df_trans_all_fix <- df_CpGsite_gill_all %>%
  left_join(transDM_gill, by = c("Chr", "Locus")) %>%
  mutate(Meth = if_else(!is.na(logFC), 1, Meth)) %>%
  select(Chr,Locus, Meth,gene_id) %>%  #REMOVING INTERGENIC FEATURE WHICH NO GENE ID IS ASSOCIATED WITH
  na.omit()
...
#only include go terms that have at least 3 genes
